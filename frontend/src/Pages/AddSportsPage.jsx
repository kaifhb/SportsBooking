...
const AddSportsPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${backURL}/api/sport/addSport`, { ...formData, centre: centreId }, {
        headers: {
          authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      setLoading(false);
      navigate(`/allSports?centreId=${centreId}`);
    } catch (err) {
      setLoading(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-indigo-100 py-12 px-6">
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2" htmlFor="basePrice">
              Base Price
            </label>
            <input
              type="number"
        ...
        <button
          onClick={() => navigate(`/centrePage?centreId=${centreId}`)}
